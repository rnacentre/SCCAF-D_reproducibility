...
p1 <- ggscatter(mydata, x = "observed_values", y = "expected_values",size = 5,ylab='',xlab = '',color = 'CT',palette = cols,
  ...
  theme(axis.line.x = element_line(colour = "black",size = 1),axis.line.y = element_line(colour = "black",size = 1),panel.grid.major = element_blank(),panel.grid.minor=element_blank(),axis.text.x = element_text(family = 'Arial',size = 30,colour = 'black'),
        ...
        axis.title.y = element_text(family = 'Arial',size = 30),strip.text.x = element_text(family = 'Arial',size=30,colour = 'black'),strip.background = element_rect(colour = 'white',fill = 'white'),legend.text = element_text(size=30),legend.title = element_text(size=30))
##adjust axis
p1+facetted_pos_scales(y=list(reference=='Sc1'~scale_y_continuous(breaks = seq(0,0.5,0.2)),
                              reference=='Sc2'~scale_y_continuous(breaks = seq(0,0.5,0.2)),
                              reference=='Sc3'~scale_y_continuous(breaks = seq(0,0.5,0.2)),
                              reference=='Sc4'~scale_y_continuous(breaks = seq(0,0.5,0.2)),
                              reference=='Opt. Ref'~scale_y_continuous(breaks = seq(0,0.5,0.2))
),
x=list(reference=='Sc1'~scale_x_continuous(breaks = seq(0,0.5,0.25)),
       reference=='Sc2'~scale_x_continuous(breaks = seq(0,0.6,0.25)),
       reference=='Sc3'~scale_x_continuous(breaks = seq(0,0.9,0.4)),
       reference=='Sc4'~scale_x_continuous(breaks = seq(0,0.8,0.4)),
       reference=='Opt. Ref'~scale_x_continuous(breaks = seq(0,0.5,0.2))
))
dev.off()
