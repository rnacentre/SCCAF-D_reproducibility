...
# )
###modify name
# colnames(dat_rlr) <- gsub('Marquina','Marquina \n -Sanchez',colnames(dat_rlr))
# rownames(dat_rlr) <- gsub('Marquina','Marquina \n -Sanchez',rownames(dat_rlr))
rownames(dat_rlr) <- gsub('Reference','SCCAF-D',rownames(dat_rlr))
new_order <- c('SCCAF-D',"Arunachalam","Lee","Schulte","Wilk")
...
dat_rlr <- dat_rlr[-1,]
###
cairo_pdf(filename = 'pbmc_heatmap_all.pdf',width = 26,height = 6,family = 'Arial')
...
ht_list = p1+p2+p3+p4+p5+p6
draw(ht_list, ht_gap = unit(2, "cm"))

dev.off()
####
ggplot(data1)+geom_bar(aes(x=pseudobulk,y=1/RMSE,fill=group),stat = "identity",position = "dodge",width = 0.7)+scale_fill_manual(values = pal_npg()(5),#modify legend title text
...
###
rlr$group <- factor(rlr$group,levels = c('Arunachalam','Lee','Schulte','Wilk','Reference'))
rlr$group <- gsub('Reference','SCCAF-D',rlr$group)

cairo_pdf(filename = 'pbmc_dwls_rmse.pdf',width = 7,height = 6,family = 'Arial')
